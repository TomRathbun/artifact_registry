...
d2ebfaa0-24ef-4262-9b1f-6a060df6b5f5	a1573933-ec35-4bbd-a94c-e0fedbd2581d	Example		artifact_graph	{"area": "AIC2"}	2025-12-17 14:20:03.179675+04	2025-12-17 14:20:15.614517+04	\N
...
Tech Refresh Phase 2 Report		TEXT		\N	TR2-GLOBAL-DOC-005	DRAFT	GLOBAL	2025-12-17 13:43:48.319968	2025-12-17 15:24:19.016803	a1573933-ec35-4bbd-a94c-e0fedbd2581d	# TR2 Progress Report\n\n## Executive Summary\n\nThe TR2 effort aims to extend EADGE-T's capabilities beyond TR1's foundational enhancements, focusing on advanced features like:\n\n1. **AI-Augmented Command and Control**, \n2. **Modernized Remote Sites**\n3. **Remote Access and Mobile Enclaves**\n4. **Extensible Communications**\n5. **DevSecOps and MLOps**\n\nHowever, due to ongoing TR1 contracting challenges (e.g., unresolved core and non-core options, pricing concerns), the TR2 starting point remains undefined. This report emphasizes the process for scope definition and requirements elicitation, rather than preliminary content, which has not been vetted internally or with AFAD/Operators.\n\nThe MBSE-driven process starts with global **Vision** statement with a refined **Vision** for each area. **Needs** are derived from the area **Vision** at three levels: Mission, Enterprise, and Technical. **Need** descriptions follow a specific gramar: \n\n````\nAs <stakeholder>,  we need  <the need>, so that <the benefit>. \n````\n\n**Use Cases** are derived from the **Needs** with the purpose of illustrating what the needs entails at a scenario level. Given a validated **Use Case**, then Stakeholder level **Requirements** can be created. Since requirements are often unclear, they are written with aspecific grammer following the EARS syntax\n\n````\n**Unconditional**: The system shall <always true behavior>\n**Event-Driven**: When <event>, the system shall <response>\n**State-Driven**: While in <state> , the system shall <true behavior>\n**Unwanted Behavior**: If <behavior>, then the system shall <response>\n**Optional Features**: "Where <condition>, the system shall <behavior>\n````\n\nThe main purpure of the customer workshops is to validate the artifacts (**Vision**, **Needs**, **Use Cases**) and then create the **Requirements**, see** Figure 1**\n\n```mermaid\nflowchart TD\n    A[Vision: Global] -->|refines| B[Vision: Area]\n    B -->|Derive from| C[Needs: Mission, Enterprise, Technical Levels]\n    C -->|Elaborates| D[Use Cases: Trigger, Preconditions, Steps, Post Conditions]\n    D -->|Derive from| E[Requirements: Stakeholder, System, Subsystems]\n    subgraph **Artifact Refinement Process**\n        B\n        C\n        D\n        E\n    end\n    F[Customer Workshops] -.->|Refine & Validate| B\n    F -.->|Refine & Validate| C\n    F -.->|Refine & Validate| D\n    F -.->|Create & Validate| E\n    G[Traceability & Linkages] -.-> A\n    G -.-> B\n    G -.-> C\n    G -.-> D\n    G -.-> E\n\nclassDef external fill:#f9f,stroke:#333,stroke-width:2px\n    class F external\n```\n**Figure 1: Process Flow for TR2 Requirements Elicitation**\n\n## Artifact Registry Motivation\n\nThe Artifact Registry was developed to overcome the inefficiencies of spreadsheet-based artifact management, which struggles with relational data, visual integration, and traceability in large-scale systems engineering efforts like TR2. By leveraging a structured PostgreSQL database, FastAPI backend, and Node.js frontend (See** Figure 2**), it provides a vertical, intuitive interface for displaying related information, seamless linking between artifacts (e.g., visions to needs via JSON imports), embedded graphics (e.g., Mermaid diagrams for use cases), and hyperlinks to external resources (e.g., the EADGE-T TR1 Proposal PDF). This not only streamlines requirements elicitation and collaboration but acts as a force multiplier, ensuring traceability and consistency across the engineering lifecycle—critical when TR1 baseline uncertainties (e.g., unresolved core/non-core options from the Lockheed Martin proposal dated 15 November 2025) delay customer engagements for TR2 scope definition.\n\n```mermaid\nflowchart TD\n    A[User] -->|HTTP| B["React Client: Port 5173"]\n    C[User] --> |HTTP| B\n    B -->|JSON / REST| FastAPI["FastAPI Backend: Port 8000"]\n    FastAPI -.->|Generates| Docs["API Docs: /docs"]\n    FastAPI -->|SQL| PG["PostgreSQL: Port 5432"]\n```\n**Figure 2: Artifact Registry Architecture**\n\nOnce TR1 baseline is confirmed, we can engage stakeholders to refine TR2 artifacts. Current progress includes initial drafts for key areas (e.g., AIC2), demonstrating the process's viability.\n1. Background on TR1 and TR2 Uncertainties\nTR1, as detailed in the Lockheed Martin Technical Proposal (dated November 15, 2025), addresses obsolescence and foundational upgrades to sustain EADGE-T's defensive effectiveness. Key elements include:\n\nRefresh Categories: Cybersecurity (e.g., POA&M remediation, SIEM for continuous monitoring), Compute Infrastructure (e.g., server hardware refresh to VMware Cloud Foundation), Network Infrastructure (e.g., 3-stage data center architecture with Juniper switches), and mission application updates (e.g., CommandIQ integration).\nCore and Non-Core Options: Core options focus on essential cybersecurity and infrastructure uplifts (e.g., endpoint protection, vulnerability scanning). Non-core options add advanced capabilities like AI/ML threat detection and new sensor integration (see Table 1 for summary).
\.
