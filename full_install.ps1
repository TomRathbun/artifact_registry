...
Write-Host ""
Write-Host "--- Artifact Registry: Automated Installation ---" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "[2/6] Initializing PostgreSQL environment..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[3/6] Setting up external data storage (registry-data)..." -ForegroundColor Yellow
...
Write-Host "  [OK] Data folders created at: $DataRoot"

# 4. Generate Environment Configuration
Write-Host ""
Write-Host "[4/6] Configuring Environment Files (.env)..." -ForegroundColor Yellow
...
if (-not (Test-Path "frontend/.env")) {
    ...
    Write-Host "  [OK] Generated frontend/.env"
}
...
Write-Host ""
Write-Host "[6/6] Finalizing Database..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "  [OK] Registry database created."
}
else {
...
uv run scripts/initialize_database.py
